...
const Datatable = () => {
  ...
  const { isLoading, err, data } = useQuery(["instructors"], () =>
  ...
  const deletemutation = useMutation(
    (deleted) => {
      return makeRequest.delete("instructors/delete/" + deleted);
    },
    {
      onSuccess: () => {
  ...
  const Usercolumns = [
    ...
    { field: 'profession', headerName: 'Position', width: 170, },
    ...
    field: "action", headerName: "Action", width: 200, sortable: false, renderCell: (params) => {
      return (
        <div className=" flex items-center gap-3">
          <Link to={'/instructors?id=' + params.row._id} >
      ...
      <div className='overflow-auto w-full flex flex-col dark:border-orange-600'>
        {
          isLoading ? <div className='custom-loader self-center mt-28'></div>:
            err ? <div className='self-center mt-28'>Something went wrong</div>
              : <DataGrid
              rows={data || []}
